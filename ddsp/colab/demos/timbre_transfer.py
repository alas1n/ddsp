...
from IPython import get_ipython  # mine
...
get_ipython().system('pip install -qU ddsp')
...
# @param ['Violin', 'Flute', 'Flute2', 'Trumpet', 'Tenor_Saxophone', 'Upload your own (checkpoint folder as .zip)']
model = 'Violin'
MODEL = model
...
if model in ('Violin', 'Flute', 'Flute2', 'Trumpet', 'Tenor_Saxophone'):
    ...
    get_ipython().system('rm -r $PRETRAINED_DIR &> /dev/null')
    get_ipython().system('mkdir $PRETRAINED_DIR &> /dev/null')
    GCS_CKPT_DIR = 'gs://ddsp/models/timbre_transfer_colab/2020-11-06'
    model_dir = os.path.join(GCS_CKPT_DIR, 'solo_%s_ckpt' % model.lower())

    get_ipython().system('gsutil cp $model_dir/* $PRETRAINED_DIR &> /dev/null')
    ...
    for fnames in uploaded_files.keys():
        ...
        get_ipython().system('unzip -o "/content/$fnames" -d $UPLOAD_DIR &> /dev/null')
